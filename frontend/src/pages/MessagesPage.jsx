...
function MessagesPage() {
  ...
  useEffect(() => {
    ...
    const handleMessagesSeen = (data) => {
      if (
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.sender === currentUser._id && !m.seen
  ...
  return (
    <Screen
      middleScreen
      className="!p-0 !pb-14 md:!pb-0 flex flex-col md:flex-row h-[100dvh] md:h-screen overflow-hidden"
    >
      {/* LEFT COLUMN: List */}
      <div
        className={`w-full md:w-[40%] h-full flex flex-col border-r border-gray-200 dark:border-gray-800 ${isChatActive ? "hidden md:flex" : "flex"}`}
      >
        <PageHeader title="Messages" className="p-4" />
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      <p
                        className={`truncate ${conv.unreadCount > 0 ? "font-bold theme-text" : "text-gray-500"}`}
                      >
                        {conv.lastMessage
      ...
      <div
        className={`w-full md:w-[60%] h-full flex flex-col ${!isChatActive ? "hidden md:flex" : "flex"}`}
      >
        {activeConversation ? (
          <>
            {/* Chat Header */}
                ...
                <>
                  {messages.map((msg, index) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`max-w-[75%] px-4 py-2 rounded-2xl ${isMine ? "bg-primary-light-accent dark:bg-primary-dark-accent text-white rounded-br-sm" : "bg-gray-200 dark:bg-gray-800 text-black dark:text-white rounded-bl-sm"}`}
                        >
